...
const GridObj = {
  categories: [
          ...
          },
          {
            ...
            Image: ["cornbread-salad.webp" , "Cornbread-Salad_2.webp"],
            Description: [
...
const FoodRecipesPage = () => {
  ...
  const Image = $(
    `<img class="BigsourceImg" src="${GridObj["categories"][0]["Image"][0]}" alt="${GridObj["categories"][0]["title"]}" />`
  );
  hyperLink.append(Image);
  const titleBigImg = $(` <div class="titleBigImg"></div>`);
  bigImage.append(titleBigImg);
  const para = $(
    ` <p>${GridObj["categories"][0]["title"]}</p>`
  );
  ...
  hyperLink.on("click", function () {
   ...
   // const title = $(this).find(".titleSmallImg").text();
    GridObj["categories"].forEach(function(ele){
if (ele.title === title){
    arr2.push(ele)
}
    })
  ...
  for (
    ...
    const ImageSmallimg = $(
      `<img class="imgSmall" src="${GridObj["categories"][i]["Image"][0]}" alt="${GridObj["categories"][i]["title"]}" />`
    );
    hyperLinkSmallImg.append(ImageSmallimg);
    const titleSmallImg = $(`<div class="titleSmallImg">`);
    item.append(titleSmallImg);
    const paraSmall = $(
      ` <p>${GridObj["categories"][i]["title"]}</p>`
    );
...
if (ele.title === title){
  ...
  for (
    let i = 0;
    i < dinnerSuggestions["categories"].length;
    i++
  ) {
    const card = $(`<div class="card"></div>`);
    gridCards.append(card);
    const imgCard = $(
      ` <img class="imgCard" src="${dinnerSuggestions["categories"][i]["Image"][0]}" alt="${dinnerSuggestions["categories"][i]["title"]}">`
    );
    card.append(imgCard);
    ...
    const titleOfRecipe = $(
      `<h4 class="titleOfRecipe"><b>${dinnerSuggestions["categories"][i]["title"]}</b></h4>`
    );
    divTitleOfRecipe.append(titleOfRecipe);
    const DivOfRate = $(`<div class ="RateFav"></div>`);
    const rate = $(
      `<p class = "RateText">Rate : ${dinnerSuggestions["categories"][i]["Rate"]}</p>`
    );
  ...
  $(".titleOfRecipe").on("click", function (event) {
    const DesOfCard = $(this).text();
    dinnerSuggestions.categories.forEach(function (x) {
       // console.log(x)
      //  console.log(DesOfCard)
        if (x.title.toLowerCase() === (DesOfCard.toLowerCase())) {
                arr2.push(x);
              }
    //   for (const keys in x) {
        //     return x[keys].filter(function(ele){
        ...
        //     })
        // x[keys].forEach(function (ele) {
        //   if (ele.title.toLowerCase().startsWith(DesOfCard.toLowerCase())) {
        //     arr2.push(ele);
        //   }
        // });
     // }
  ...
  for (
    let i = 0;
    i < quickDinner["categories"].length;
    i++
  ) {
    const card = $(`<div class="card"></div>`);
    gridCards2.append(card);
    const imgCard = $(
      ` <img class="imgCard" src="${quickDinner["categories"][i]["Image"][0]}" alt="${quickDinner["categories"][i]["title"]}">`
    );
    card.append(imgCard);
    const container = $(`<div class="container"></div>`);
    card.append(container);
    const divTitleOfRecipe = $(`<div class = "divTitleOfRecipe"></div>`);
    const titleOfRecipe = $(
      ` <h4 class="titleOfRecipe"><b>${quickDinner["categories"][i]["title"]}</b></h4>`
    );
    divTitleOfRecipe.append(titleOfRecipe);
    const DivOfRate = $(`<div class ="RateFav"></div>`);
    const rate = $(
      `<p class = "RateText">Rate : ${quickDinner["categories"][i]["Rate"]}</p>`
    );
    const fav = $(
      `<button class="fav" ><img src="heart-fill.svg" alt="Favorite" /></button>`
    );
...
$(".titleOfRecipe").on("click", function (event) {
    const DesOfCard = $(this).text();
    quickDinner.categories.forEach(function (x) {
       // console.log(x)
      //  console.log(DesOfCard)
        if (x.title.toLowerCase() === (DesOfCard.toLowerCase())) {
                arr2.push(x);
  ...
  inputSearch.on("click", function () {
    if (inputSearch.val() == "") {
      ...
      const h3 = $(
        ` <div class = "SearchHeader"><h3 class ="favHeadertext">Your Search :</h3></div>`
      );
      ...
      arr.forEach(function (ele, index) {
        ...
        divTitleOfRecipe.append(titleOfRecipe);
        const DivOfRate = $(`<div class ="RateFav"></div>`);
        const rate = $(`<p class ="RateText">Rate ${ele.Rate}</p>`);
...
$(".titleOfRecipe").on("click", function (event) {
  ...
  const footerSubscribeContainer = $(
    `<div class="footer-subscribe--container"></div>`
  );
...
const FavoritePage = () => {
  ...
  const h3 = $(
    ` <div class = "favHeader"><h3 class ="favHeadertext">Your Favorite :</h3></div>`
  );
  const cardFav = $(`<div class ="cards"></div>`);
  ...
  if (favArr.length !== 0) {
    ...
    favArr.forEach(function (e, index) {
      ...
      divTitleOfRecipe.append(titleOfRecipe);
      const DivOfRate = $(`<div class ="RateFav"></div>`);
      const rate = $(`<p class ="RateText">Rate ${e["rate"]}</p>`);
      const buttonFav = $(
        `<button class="fav" ><img src="heart-fill.svg" alt="Favorite" /></button>`
      );
...
const CategoryPage = () => {
  ...
  ButtonFilter.on("click", function () {
    ...
    const btn3 = $(
      `<button class ="ButtonSalads buttonfilter">Salads</button>`
    );
    const btn4 = $(
  ...
  for (let i = 0; i < Recipes["categories"][0]["Spaghetti"].length; i++) {
    ...
    const imgCardOne = $(
      `<img class="imgCard" src="${Recipes["categories"][0]["Spaghetti"][i]["Image"][0]}"alt="${Recipes["categories"][0]["Spaghetti"][i]["title"]}">`
    );
    ...
    const DivOfRate = $(`<div class ="RateFav"></div>`);
    const rateOne = $(
      `<p class = "RateText">Rate : ${Recipes["categories"][0]["Spaghetti"][i]["Rate"]}</p>`
    );
    const favOne = $(
      `<button class="fav" ><img src="heart-fill.svg" alt="Favorite" /></button>`
    );
  ...
  for (let i = 0; i < Recipes["categories"][0]["Salads"].length; i++) {
    ...
    const imgCardOne = $(
      `<img class="imgCard" src="${Recipes["categories"][0]["Salads"][i]["Image"][0]}"alt="${Recipes["categories"][0]["Salads"][i]["title"]}">`
    );
    cardOne.append(imgCardOne);
    const containerOne = $(`<div class="container"></div>`);
    cardOne.append(containerOne);
    ...
    const titleOfRecipeOne = $(
      ` <h4 class="titleOfRecipe"><b>${Recipes["categories"][0]["Salads"][i]["title"]}</b></h4>`
    );
    const DivOfRate = $(`<div class ="RateFav"></div>`);
    divTitleOfRecipe.append(titleOfRecipeOne);
    const rateOne = $(
      `<p class = "RateText">Rate : ${Recipes["categories"][0]["Salads"][i]["Rate"]}</p>`
    );
    const favOne = $(
      `<button class="fav" ><img src="heart-fill.svg" alt="Favorite" /></button>`
    );
  ...
  for (let i = 0; i < Recipes["categories"][0]["Chicken"].length; i++) {
    ...
    const imgCardOne = $(
      `<img class="imgCard" src="${Recipes["categories"][0]["Chicken"][i]["Image"][0]}"alt="${Recipes["categories"][0]["Chicken"][i]["title"]}">`
    );
    cardOne.append(imgCardOne);
    const containerOne = $(`<div class="container"></div>`);
    cardOne.append(containerOne);
    const divTitleOfRecipe = $(`<div class = "divTitleOfRecipe"></div>`);
    const titleOfRecipeOne = $(
      ` <h4 class="titleOfRecipe"><b>${Recipes["categories"][0]["Chicken"][i]["title"]}</b></h4>`
    );
    divTitleOfRecipe.append(titleOfRecipeOne);
    const DivOfRate = $(`<div class ="RateFav"></div>`);
    const rateOne = $(
      `<p class = "RateText">Rate : ${Recipes["categories"][0]["Chicken"][i]["Rate"]}</p>`
    );
    const favOne = $(
      `<button class="fav" ><img src="heart-fill.svg" alt="Favorite" /></button>`
    );
...
const FoodRecipeDetails = (ArrayOfObj) => {
  body.html("");
  ...
  const TitleOfDetailsFoodh3 = $(
    `<h3 class ="TitleOfDetailsFoodh3">${ArrayOfObj[0]["title"]}</h3>`
  );
  TitleOfDetailsFood.append(TitleOfDetailsFoodh3);
  const paraOne = $(
    ` <div class ="paraOne">${ArrayOfObj[0]["Description"][0]}</div>`
  );
  const divImgOfDetailsFood = $(`<div class = "divImgOfDetailsFood"></div>`);
  const imgOfDetailsFood = $(
    `<img class ="imgOfDetailsFood" src=${ArrayOfObj[0]["Image"][0]} alt =${ArrayOfObj["title"]}>`
  );
  divImgOfDetailsFood.append(imgOfDetailsFood);
  const paraTwo = $(
    ` <div class ="paraTwo">${ArrayOfObj[0]["Description"][1]}</div>`
  );
  const divImgOfDetailsFoodTwo = $(`<div class = "divImgOfDetailsFood"></div>`);
  const imgOfDetailsFoodTwo = $(
    `<img class ="imgTwoOfDetailsFood imgOfDetailsFood" src=${ArrayOfObj[0]["Image"][1]} alt =${ArrayOfObj[0]["title"]}>`
  );
  divImgOfDetailsFoodTwo.append(imgOfDetailsFoodTwo);
  const paraThree = $(
    ` <div class ="paraThree">${ArrayOfObj[0]["Description"][2]}</div>`
  );
  FoodRecipeDetails.append(
    TitleOfDetailsFood,
    paraOne,
    divImgOfDetailsFood,
    paraTwo,
    divImgOfDetailsFoodTwo,
    paraThree
  );
